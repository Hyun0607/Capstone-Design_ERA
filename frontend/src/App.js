...
function App() {
  ...
  const pageSize = 3; // 한 페이지에 보여줄 아이템 수
  // ✅ 다음 숙소 보기 버튼 (>)
  const handleNext = () => {
  const nextOffset = viewRange + pageSize;
  if (nextOffset >= response.length) {
    alert('더 이상 숙소가 없습니다.');
    return;
  }
   setViewRange(nextOffset);
  };
  ...
  setViewRange(viewRange - pageSize);
};
